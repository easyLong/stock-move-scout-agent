...
$pidPath = Join-Path $root "runs\stock_scout_scheduler.pid"
$logPath = Join-Path $logDir "stock_scout_scheduler.log"
$errPath = Join-Path $logDir "stock_scout_scheduler.err.log"
...
if (Test-Path $pidPath) {
    ...
    if ($oldPid -and (Get-Process -Id $oldPid -ErrorAction SilentlyContinue)) {
        Write-Output "stock_scout_scheduler already running. PID=$oldPid"
        exit 0
    }
}
...
Write-Output "Started stock_scout_scheduler. PID=$($process.Id)"
