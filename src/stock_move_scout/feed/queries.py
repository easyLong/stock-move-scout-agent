...
def intel_feed_sql(trade_date: str | None = "") -> str:
    ...
    scan_ranked AS (
      SELECT
        ...
        JSON_MERGE_PRESERVE(
          ...
          IF(JSON_VALID(lhb.key_facts) AND JSON_LENGTH(lhb.key_facts) > 0, JSON_ARRAY(JSON_OBJECT(
              ...
              'label', '龙虎榜席位',
              'type', 'lhb',
              'source', CONCAT('当日龙虎榜 / ', lhb.seat_signal_label),
          ...
          IF(JSON_VALID(smj.support_items) AND JSON_LENGTH(smj.support_items) > 0, JSON_ARRAY(JSON_OBJECT(
              ...
              'type', 'support',
              'source', '判断引擎',
              'body', CONCAT_WS('\n', smj.support_items->>'$[0]', smj.support_items->>'$[1]', smj.support_items->>'$[2]'),
              'priority', 3,
              'payload', JSON_EXTRACT(smj.support_items, '$')
            )), JSON_ARRAY()),
          IF(JSON_VALID(smj.score_detail)
            AND (
              JSON_SEARCH(JSON_EXTRACT(smj.score_detail, '$.anchor_leadership_reasons'), 'one', '近%') IS NOT NULL
              OR JSON_SEARCH(JSON_EXTRACT(smj.score_detail, '$.anchor_leadership_reasons'), 'one', '问财%') IS NOT NULL
              ...
              'source', '问财区间排名',
              'body', CONCAT_WS('\n',
                JSON_UNQUOTE(JSON_EXTRACT(smj.score_detail, '$.anchor_leadership_reasons[0]')),
                JSON_UNQUOTE(JSON_EXTRACT(smj.score_detail, '$.anchor_leadership_reasons[1]')),
                JSON_UNQUOTE(JSON_EXTRACT(smj.score_detail, '$.anchor_leadership_reasons[2]'))
              ),
              'priority', 3,
              'payload', JSON_EXTRACT(smj.score_detail, '$.anchor_leadership_reasons')
            )), JSON_ARRAY()),
      ...
      LEFT JOIN stock_move_judgements smj ON smj.event_type='realtime' AND smj.event_id=sr.run_id AND smj.code = sm.code
      LEFT JOIN async_evidence_summaries aes ON aes.trade_date={day} AND aes.code = sm.code
      LEFT JOIN stock_lhb_seat_evidence lhb ON lhb.trade_date={day} AND lhb.code = sm.code
    ...
    window_ranked AS (
      ...
      LEFT JOIN stock_move_judgements smj ON smj.event_type='stable' AND smj.event_id=rw.window_id AND smj.code = wm.code
      LEFT JOIN async_evidence_summaries aes ON aes.trade_date={day} AND aes.code = wm.code
      LEFT JOIN stock_lhb_seat_evidence lhb ON lhb.trade_date={day} AND lhb.code = wm.code
    ...
    FROM (
      SELECT event_time, kind, kind_label, code, name, sort_rank, title, summary, detail, evidence_items, display_contract, score, change_pct, speed_pct, amount_yi, tags
      FROM scan_ranked
      WHERE rn <= 5
